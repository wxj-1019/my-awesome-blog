...
def main():
    ...
    print("步骤 2: 完全清理并重新克隆...")
    print("  删除旧目录...")
    ssh_exec(client, f"rm -rf {DEPLOY_PATH}")
    
    print("  克隆代码仓库...")
    code, out, err = ssh_exec(client, f"git clone {REPO_URL} {DEPLOY_PATH}", timeout=120)
    if code != 0:
        print(f"  克隆错误: {err}")
        return
    print("  代码克隆完成!")
    ...
    print("步骤 3: 检查关键文件...")
    code, out, err = ssh_exec(client, f"ls -la {DEPLOY_PATH}/frontend/src/components/ui/ | head -20")
    print(out)
    print()
...
# 检查文件
RUN ls -la src/components/ui/ | head -20
