...
class ImageGenTaskRequest(BaseModel):
    ...
    model: str = Field(default="rhart-image-g-2-official", pattern=r"^[A-Za-z0-9._-]+$", description="图片生成模型标识")
    # 图片生成模式：text 文生图 / image 图生图（配合 image_urls 使用）
    mode: Literal["text", "image"] = Field(default="text", description="图片生成模式")
    # 图生图参考图 URL 列表（mode=image 时应提供；缺失时 RunningHub 会拒绝，键名为 imageUrls）
