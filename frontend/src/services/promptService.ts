...
export const promptService = {
  ...
  async deleteFolder(id: string, moveToFolderId?: string): Promise<void> {
    const queryParams = moveToFolderId ? `?move_to=${moveToFolderId}` : '';
    return apiRequest<void>(`${API_BASE}/folders/${id}${queryParams}`, {
      method: 'DELETE',
    });
  },
  async exportPrompts(promptIds?: string[]): Promise<PromptExportData> {
    const queryParams = promptIds && promptIds.length > 0
      ? `?ids=${promptIds.join(',')}`
      : '';
