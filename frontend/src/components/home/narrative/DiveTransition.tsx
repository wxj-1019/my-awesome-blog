 ...
 * - 光柱：自上而下径向渐变，GSAP scrub 淡入下沉（桌面）；移动 / RM 静态
    ...
    >
      ...
      >
        ...
        <div
          ...
          className="absolute inset-x-0 top-0 h-2/5 bg-gradient-to-b from-transparent via-primary/[0.05] to-primary/[0.08]"
        />
        {/* 色带 · 中：水体过渡（主色 → 深水蓝） */}
        <div
          data-dive-band="mid"
          className="absolute inset-x-0 top-1/3 h-1/3 bg-gradient-to-b from-primary/[0.08] via-primary/[0.06] to-tech-deepblue/15"
        />
        {/* 色带 · 下：水下定调（深水蓝收进背景；桌面由 GSAP 灌至全深） */}
        <div
          data-dive-band="deep"
          className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-b from-tech-deepblue/15 via-tech-deepblue/10 to-background"
        />

        {/* 主折光线：opacity 仅由 GSAP 写入（见 useGSAP）；hydration 前透明避免闪一下 */}
        <div
          data-dive-shimmer="primary"
          className="absolute inset-x-0 top-[30%] h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent"
          style={{ opacity: 0 }}
        />
        {/* 副折光线：更细更淡，静态（不走 GSAP） */}
        <div
          data-dive-shimmer="secondary"
          className="absolute inset-x-[15%] top-[55%] h-px bg-gradient-to-r from-transparent via-tech-lightcyan/20 to-transparent"
        />

        {/* 光柱：径向渐变成锥形；opacity/y 仅 GSAP；mx-auto 不与 y 抢 transform */}
        <div
          data-dive-lightshaft
          className="absolute inset-x-0 top-0 mx-auto h-full w-[46%]"
          style={{
            opacity: 0,
            background:
              'radial-gradient(ellipse 55% 100% at 50% 0%, color-mix(in srgb, var(--primary) 22%, transparent), transparent 72%)',
          }}
        />
