...
export default function StatsGrid({ stats, loading }: StatsGridProps) {
  ...
  return (
    <GlassCard
      padding="lg"
      className="border-tech-cyan/20 hover:shadow-[0_0_30px_var(--shadow-tech-cyan)] transition-colors duration-300"
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statsData.map((stat) => (
          <div
            key={stat.label}
            className="group relative p-4 rounded-xl bg-[var(--card)]/40 hover:bg-tech-cyan/10 transition-[colors,transform] duration-300 hover:scale-105 cursor-pointer"
          >
            {/* 渐变边框效果 */}
