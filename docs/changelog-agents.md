...
| 日期 | 问题 | 状态 |
|------|------|------|
| 2026-09-09 | 全项目代码审查优化（按优先级 3 批） | 已实施。**高优**：①statistics_service 分类/标签/作者统计改单次 GROUP BY（消除 N+1）+ crud/article 建文标签循环改 `in_` 查询；②monitoring 接口假数据改真实来源——活跃连接走请求中间件计数、缓存命中率/响应时间读 Redis INFO+ping、analytics 总量/今日/错误率/平均耗时/热门端点走中间件累计（by_path 上限 500 防膨胀）、active_users 查库、cache_size 走 dbsize，health 数据库检查改真实 SELECT 1；③4 处裸 `except:` 改精确捕获并记日志；④auth/users/conversations 的 async 端点同步 CRUD 调用包裹 `asyncio.to_thread`，email_service 新增 `send_email_async`（smtplib 阻塞调用隔离线程池，当前无调用方备用）。**中优**：⑤DEFAULT_TENANT_ID + 6 档 LLM temperature + WEATHER_DEFAULT_CITIES 收敛进 settings；⑥混用文件（articles/memories/writing_sessions）HTTPException 收敛到自定义异常体系，articles/writing_sessions 权限判断收敛到 permission_helpers（conversations 因字段语义不同、_can_view_article 谓词形态等 3 处跳过）；⑦前端 truncate 抽至 lib/utils、编辑器 onApplyRevision/插值逻辑抽共享模块（admin/articles/lib/apply-revision.ts）、SettingsView any 修复、admin 首页 useEffect 依赖修复、videos 列表分批渲染（每批 20 + 加载更多）、弹幕同屏 50 条硬上限（含 state 配套裁剪防 onRemove 泄漏）。**低优**：⑧crud/service 层 7 处类型注解补全、endpoints 下 20+ 处未使用 import 清理；⑨修复存量 f-string 语法错误 `app/prompts/base.py:45`。验证：后端 296 passed / 4 skipped（PG 全文搜索），前端 tsc 0 error、eslint 0 error（10 warnings 均为存量） |
...
| 2026-07-26 | chat 顶栏 Menu 图标按钮冗余；工具弹窗长列表居中偏上溢出 | 已修复：移除顶栏 Menu 按钮（侧栏桌面端常驻 / 移动端走自身遮罩），清理 `onToggleSidebar` 与 `Menu` import；弹窗居中由 `translate` 定位改为遮罩层 `flex items-center justify-center` 容器，避免 `max-h` 长列表配合 `translate-y` 时顶部溢出视口 |
 | 2026-09-08 | 后端安全/质量修复一批（A1-A4、B1-B4） | 已实施：①A1 `DELETE /oss/delete` 改 `get_current_superuser`，堵住任意登录用户删任意 file_url（新建 `test_oss_upload.py` 补权限与冒烟测试）；②A2 `images.py` 修复 except Exception 吞掉 400 变 500、上传改走 `save_upload_file_temp()` 统一校验、搜索 ilike 转义 `%`/`_` 加 `ESCAPE '\'`（顺带修复搜索引用不存在的 title/description 字段致任意搜索 500）；③A3 评论列表 `approved` 参数仅超级管理员可用，匿名/普通用户强制只看已审核；④A4 文生视频任务与 RunningHub 账户信息改 `get_current_active_user`（生图保持公开+限流）；⑤B1 分类/标签/作者统计改单条 GROUP BY 聚合消除 N+1；⑥B2 全 endpoints 裸 `skip`/`limit` 补 `Query(ge/le)` 校验；⑦B3 `.env.example` 按 `config.py` Settings 重写（删 EMAIL_HOST 等死字段，补 MINIO_*/RUNNINGHUB_*/COVER_SOURCE/PEXELS/UNSPLASH/ALAPI/LANGCHAIN_*/FRONTEND_URL/RATE_LIMIT_*/CACHE_*/UPLOAD_*）；⑧B4 删除 `test_fulltext_search.py`——4 个函数体全 pass 且无断言，skipif 使 CI 永远跳过，制造「已覆盖」假象，待 PG CI 就绪后重建真实用例 |
| 2026-09-08 | 后端 jieba 全文搜索上线但前端零接入；Navbar 搜索按钮聚焦不存在的 `global-search-input` 是死按钮 | 已上线全局搜索弹窗（分支 feat/global-search）：新建 `lib/api/search.ts`（`searchArticlesFulltext`，走 apiFetch + AbortSignal）与 `components/navigation/GlobalSearch.tsx`（Radix Dialog、300ms 防抖、AbortController 竞态保护、↑↓/Enter/Esc 键盘导航、加载/错误/空态/引导四态、z-index 110 压过导航栏）；Navbar 桌面按钮 + Cmd/Ctrl+K + 移动端菜单三入口接入并删除 focusSearch 死代码，/admin 路由守卫防状态泄漏；修复 EmptyState 默认 action 在弹窗内触发 location.reload 的问题（显式传「清除搜索」「重试」action）；jest 57 套件 365 用例全绿 |
| 2026-09-08 | portfolio 后台管理齐全但前台无展示页；`lib/api/portfolio.ts` 死代码且类型漂移（image_url/project_url 与后端 cover_image/demo_url 不符） | 已上线 `/portfolio` 公开页（纯 RSC 零客户端 JS）：`PortfolioCard`（封面16:9/精选与状态徽章/技术栈 5+N/日期范围/demo·GitHub 外链）+ `PortfolioGrid`（`sortPortfolioItems` 精选→sort_order→created_at，空态 EmptyState）+ 页头 PageActHeader（font-creative）；Navbar 相册后加「作品集」入口（移动端自动继承）+ sitemap 收录；`lib/api/portfolio.ts` 三 interface 重写对齐后端 schema（函数体未动）；后端不可达时页面渲染空态不 500；jest 59 套件 371 用例全绿 |
| 2026-09-12 | 远端并行工作线未经审查合入；订阅推送链路全线断裂（订阅表/邮件模板/退订端点均为孤岛，发布处零触发、前端无订阅 UI） | 已实施两批：①审查远端 8 commits 修复 4 个中危——监控 by_path 改路由模板计数（原 500 上限被 UUID 路径占满后新端点永久丢失）、/health DB 检查加 2s 超时防线程池耗尽、permission_helpers 改抛 ForbiddenException 统一 error.code、api-client 错误解析对齐后端 `{"error":{"message"}}` 嵌套契约（原中文错误文案用户永远看不到）；②订阅邮件推送闭环——email_service 新文章通知改逐人发送（原 To 头泄露全部收件人）+ 退订链接真实化、新增 notification_service（BackgroundTasks 触发，自建 Session 防 get_db 会话已关）、articles POST/PUT 检测未发布→已发布触发、订阅创建发验证邮件 + POST /verify 接线、Footer 邮件订阅表单、前端新增 /verify-subscription 与 /unsubscribe 页、.env.production.example 补 SMTP 键组、响应 schema 剔除 verification_token 防替人验证；集成时修复三处契约不匹配（verify body/query、邮件链接路径/query）。验证：后端 348 passed、前端 374 passed、tsc 0 |
| 2026-09-14 | 四大迭代：①admin/articles new/[id] 编辑器各 1100+ 行重复；②services/ 与 lib/api/ 双 API 层并存；③文章无 AI 摘要能力、相关文章推荐只按分类；④文章分享无 OG 图 | 已实施四工作流（4 工程师并行 + 集成收尾 + 独立终审）：①抽 `useArticleEditorForm` + `ArticleEditorForm` 共享层，两页 1134→349、1095→320 行，行为等价（submitData 逐字段、自动保存门控、回填不标脏、润色中止、快捷键、校验文案逐字核对）；②src/services/ 目录清零——articleService 96% rename 迁 lib/api/articles、commentService（UIComment 树映射）与 interactionService 整迁、其余并入 guestbook/weather/prompts/memories/timeline-events/statistics/friend-links 或删死文件，错误读取统一 extractApiErrorMessage；③后端新增 `POST /ai/article-summary`（LLM 无状态摘要，title 可空、正文 ≥50 字、20/min 限流）+ 编辑器「AI 生成摘要」按钮，相关文章改加权评分（共享 tag×3/同分类×2/jieba 关键词×1 上限 5/近 30 天×1，SQL≤4 有引擎事件断言，候选空回退旧逻辑）；④`opengraph-image.tsx`（next/og 1200×630，NotoSansSC 入 public/fonts——**生产镜像只拷 public/**，src/assets 路径会让中文 OG 图静默降级）+ 日级 revalidate。终审修复契约项（空标题 422）。**注意：Next 16 文件约定 params 为 Promise，dev 重启需 INTERNAL_API_URL 指向本机后端**。验证：后端 361 passed、前端 378 passed、tsc 0、lint 0 error |
